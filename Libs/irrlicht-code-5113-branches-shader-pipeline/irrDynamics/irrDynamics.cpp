...
irr::core::vector3df btToIrrVec(const btVector3 &inVec) {
...
btRigidBody* irrDynamics::addTriMesh_moving(irr::scene::IMeshSceneNode* node,f32 mass,unsigned int precisionLoss,short group,short mask) { //any concave stuff will become convex
	...
	f32 ccdThreshold = std::min(Mx-mx,std::min(My-my,Mz-mz))*0.9f;
...
bool irrDynamics::rayTest(const btVector3 &Start,const btVector3 &End) {
...
irr::core::vector3df irrDynamics::rayTestPoint(const btVector3 &Start,const btVector3 &End) {
    btCollisionWorld::ClosestRayResultCallback RayCallback(Start, End);
	world->rayTest(Start,End,RayCallback);
...
irr::core::vector3df irrDynamics::rayTestNormal(const btVector3 &Start,const btVector3 &End) {
    btCollisionWorld::ClosestRayResultCallback RayCallback(Start,End);
    world->rayTest(Start,End,RayCallback);
    if (RayCallback.hasHit()) {
